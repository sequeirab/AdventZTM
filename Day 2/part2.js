...
const checkPass = (input) =>  {
    return input
        .split(/\n/)
        .reduce((validPass, currentPass) => {
                const pattern = /(\d?\d)\-(\d?\d)\s([a-z])\:\s([a-z]*)/gm;
                const matchGroup = pattern.exec(currentPass);

                const firstPosition = matchGroup[1];
                const secondPosition = matchGroup[2];
                const letter = matchGroup[3];
                const pass = matchGroup[4];
